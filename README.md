...
当前版本不依赖任何付费第三方智能体平台。默认模型服务商为通义千问（Qwen），仍可在设置页切换 DeepSeek。没有配置当前服务商密钥时，会自动使用内置演示引擎，因此克隆后即可跑通全部流程。工作台自由对话已接入同一套服务端模型配置；Career Profile 和当前 JD 默认均不发送，用户可逐项授权并在发送前预览字段。档案上下文会主动排除姓名与整份简历正文。账号与云同步为可选增强项：不配置 Supabase 时继续使用浏览器本地存储，不影响十个任务 Agent 与自由对话。
...
- 生产环境默认只允许登录用户使用服务端共享模型密钥，并通过 Supabase 原子配额同时限制账号分钟请求、每日请求、每日/月度 Token、哈希网络身份和并发任务；只有已经部署外部网关限流并明确接受匿名调用费用风险时才可设置 `ALLOW_PUBLIC_AI_API=true`。
- 共享额度调用会生成私有用量事件，记录请求编号、模型、Token、耗时、状态和按环境变量配置的成本估算，不保存简历、JD、对话正文或原始 IP。
...
`pnpm check:production` 用于检查生产环境密钥、Supabase、限流、成本单价、健康检查和告警配置；本地演示缺少这些配置属于正常情况，因此该命令会明确列出尚未完成的上线项。
