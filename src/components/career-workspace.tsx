...
export function CareerWorkspace() {
  ...
  const completion = useMemo(() => {
    const fields = [profile.basics.name, profile.basics.school, profile.basics.major, profile.basics.grade, profile.basics.targetRole];
    const filled = fields.filter(Boolean).length + (profile.skills.length ? 1 : 0) + (profile.projects.length ? 1 : 0);
        ...
        <div className="mx-auto flex h-[72px] max-w-[1440px] items-center justify-between px-5 lg:px-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] font-semibold tracking-[.18em] text-[#7e8983]">AI 求职实训空间</p>
            </div>
          </div>
        ...
        <main className="min-w-0 px-5 py-8 md:px-8 lg:px-10 lg:py-10">
          <section className="mb-8 flex flex-col justify-between gap-5 xl:flex-row xl:items-end">
            ...
            <div className="flex items-center gap-3 rounded-2xl border border-[#dce3dc] bg-white p-2 pr-4 shadow-sm">
              ...
              <div><p className="text-[11px] font-semibold text-[#89938e]">当前运行模式</p><p className="text-sm font-bold">{demoMode ? "本地演示 · 零成本" : `${provider === "deepseek" ? "DeepSeek" : "通义千问"} · 自动回退`}</p></div>
            </div>
          </section>
  ...
  return (
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-[270px] border-r border-[#e0e5e0] bg-[#eef1ed] p-5 pt-6 transition-transform lg:sticky lg:top-[72px] lg:z-0 lg:h-[calc(100vh-72px)] lg:w-auto lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <p className="mb-3 px-3 text-[10px] font-bold uppercase tracking-[.18em] text-[#929b96]">Workspace</p>
        ...
        <p className="mb-3 px-3 text-[10px] font-bold uppercase tracking-[.18em] text-[#929b96]">AI Agents</p>
        ...
        <div className="absolute bottom-6 left-5 right-5 rounded-2xl border border-[#d8e0d8] bg-white/70 p-4">
          ...
          <p className="text-[11px] leading-5 text-[#7b8780]">档案仅保存在你的浏览器，本阶段不上传数据库。</p>
        </div>
      </aside>
    </>
  );
  ...
  return (
    <section className="mb-5 grid gap-3 md:grid-cols-3">
      {agentMeta.map((item) => {
        ...
        return <button key={item.id} onClick={() => setActive(item.id)} className={`relative overflow-hidden rounded-2xl border p-4 text-left transition-all ${selected ? "border-[#2f644f] bg-[#173c30] text-white shadow-[0_12px_32px_rgba(23,60,48,.16)]" : "border-[#dee4de] bg-white hover:-translate-y-0.5 hover:border-[#b8c8bd]"}`}>
          <div className="flex items-start justify-between"><div className={`grid size-9 place-items-center rounded-xl ${selected ? "bg-white/12" : "bg-[#edf2ed] text-[#4c7160]"}`}><Icon size={17} /></div>{completed[item.id] ? <CheckCircle2 size={17} className={selected ? "text-[#b7e698]" : "text-[#5d9a6a]"} /> : <span className={`text-[10px] font-bold tracking-wider ${selected ? "text-white/45" : "text-[#adb5b0]"}`}>{item.number}</span>}</div>
          <h2 className="mt-4 text-sm font-extrabold">{item.title}</h2><p className={`mt-1 text-[11px] leading-5 ${selected ? "text-white/60" : "text-[#87908b]"}`}>{item.short}</p>
  ...
  return <PanelShell eyebrow="Agent 01 · Experience Miner" title="先认识你，再开始写简历" description="真实经历不等于实习。课程、竞赛、社团和个人作品，都是可挖掘的求职证据。">
    <form onSubmit={onSubmit} className="p-6 md:p-7">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <Field label="姓名"><input value={form.name} onChange={(e) => field("name", e.target.value)} placeholder="怎么称呼你" /></Field>
        <Field label="学校"><input value={form.school} onChange={(e) => field("school", e.target.value)} placeholder="学校名称" /></Field>
        <Field label="专业"><input value={form.major} onChange={(e) => field("major", e.target.value)} placeholder="所学专业" /></Field>
        <Field label="年级"><input value={form.grade} onChange={(e) => field("grade", e.target.value)} placeholder="如：大三" /></Field>
        <Field label="目标岗位"><input value={form.targetRole} onChange={(e) => field("targetRole", e.target.value)} placeholder="如：AI 产品实习生" required /></Field>
        <Field label="意向地点"><input value={form.location} onChange={(e) => field("location", e.target.value)} placeholder="城市 / 可远程" /></Field>
      </div>
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <Field label="技能与工具" hint="用逗号分隔"><textarea value={form.skills} onChange={(e) => field("skills", e.target.value)} rows={5} placeholder="Excel、Python、Figma…" /></Field>
        <Field label="经历素材" hint="先写事实，不用润色"><textarea value={form.experience} onChange={(e) => field("experience", e.target.value)} rows={5} placeholder="做过什么？承担什么？结果如何？" /></Field>
      </div>
...
function JDPanel({ text, setText, onSubmit, loading, jd }: { text: string; setText: (v: string) => void; onSubmit: (e: FormEvent) => void; loading: boolean; jd: JDAnalysis | null }) {
  return <PanelShell eyebrow="Agent 02 · Requirement Decoder" title="把一段 JD，拆成一张能力地图" description="粘贴完整岗位描述，Agent 会识别硬性要求、加分项、关键词和隐含优先级。">
    <form onSubmit={onSubmit} className="p-6 md:p-7">
      <Field label="岗位描述（JD）" hint={`${text.length} 字`}><textarea className="min-h-[260px] font-mono text-[13px] leading-6" value={text} onChange={(e) => setText(e.target.value)} placeholder="请粘贴招聘网站中的完整岗位描述…" required /></Field>
...
function MatchPanel({ profile, jd, match, loading, run, goTo }: { profile: CareerProfile; jd: JDAnalysis | null; match: MatchReport | null; loading: boolean; run: () => void; goTo: (id: WorkspaceAgentName) => void }) {
  ...
  return <PanelShell eyebrow="Agent 03 · Evidence Matcher" title="不是猜匹配度，而是逐条找证据" description="把学生档案与岗位要求放在同一张表上，明确能投、缺什么、先做什么。">
    ...
    {match && <ResultBlock title="岗位匹配报告"><div className="mb-7 flex flex-col gap-5 rounded-2xl bg-[#173c30] p-5 text-white sm:flex-row sm:items-center"><ScoreRing score={match.score} /><div><p className="text-[11px] font-bold uppercase tracking-[.16em] text-white/50">Match verdict</p><h3 className="mt-1 text-lg font-extrabold">{match.verdict}</h3><p className="mt-2 text-xs leading-5 text-white/60">分数用于发现差距，不代表录用概率。</p></div></div><div className="grid gap-6 md:grid-cols-2"><InfoList title="已匹配能力" items={match.matchedSkills} positive /><InfoList title="关键差距" items={match.gaps} /><InfoList title="证据依据" items={match.evidence} /><InfoList title="下一步行动" items={match.actionPlan} numbered /></div></ResultBlock>}
  </PanelShell>;
}

function ProfileRail({ profile, completion, jd }: { profile: CareerProfile; completion: number; jd: JDAnalysis | null }) {
  return <aside className="hidden border-l border-[#e0e5e0] bg-[#f0f3ef] px-5 py-8 lg:block"><div className="sticky top-[104px] space-y-4"><section className="rounded-[22px] border border-[#dce3dc] bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><p className="text-[11px] font-extrabold uppercase tracking-[.13em] text-[#7e8a83]">Career Profile</p><CircleUserRound size={18} className="text-[#698174]" /></div><div className="mt-5 flex items-center gap-3"><div className="grid size-12 place-items-center rounded-2xl bg-[#dfece1] text-sm font-black text-[#315d47]">{profile.basics.name?.slice(0, 1) || "你"}</div><div className="min-w-0"><p className="truncate text-sm font-extrabold">{profile.basics.name || "等待认识你"}</p><p className="mt-0.5 truncate text-[11px] text-[#8a948f]">{profile.basics.targetRole || "目标岗位待填写"}</p></div></div><div className="mt-5"><div className="mb-2 flex justify-between text-[11px]"><span className="font-semibold text-[#78837d]">档案完整度</span><span className="font-extrabold text-[#37664f]">{completion}%</span></div><div className="h-2 overflow-hidden rounded-full bg-[#edf0ed]"><div className="h-full rounded-full bg-[#7db477] transition-all duration-700" style={{ width: `${completion}%` }} /></div></div><div className="mt-5 space-y-3"><ProfileLine icon={GraduationCap} label={profile.basics.school || "学校待补充"} /><ProfileLine icon={BriefcaseBusiness} label={profile.basics.major || "专业待补充"} /><ProfileLine icon={MapPin} label={profile.basics.location || "地点待补充"} /></div></section><section className="rounded-[22px] border border-[#dce3dc] bg-white p-5 shadow-sm"><p className="text-[11px] font-extrabold uppercase tracking-[.13em] text-[#7e8a83]">能力标签</p><div className="mt-4"><TagList items={profile.skills.length ? profile.skills : ["完成简历后生成"]} muted={!profile.skills.length} /></div></section>{jd && <section className="rounded-[22px] bg-[#dfeadf] p-5"><div className="flex gap-3"><div className="mt-0.5 grid size-8 shrink-0 place-items-center rounded-xl bg-white/70 text-[#3d6c54]"><BriefcaseBusiness size={15} /></div><div><p className="text-[10px] font-bold uppercase tracking-[.12em] text-[#6d8376]">当前目标</p><p className="mt-1 text-sm font-extrabold text-[#294b3b]">{jd.jobTitle}</p><p className="mt-1 text-[11px] text-[#6c7c73]">{jd.company}</p></div></div></section>}</div></aside>;
}
...
function InfoList({ title, items, positive, numbered }: { title: string; items: string[]; positive?: boolean; numbered?: boolean }) { return <div><p className="mb-3 text-xs font-extrabold text-[#43544c]">{title}</p>{items.length ? <ul className="space-y-2.5">{items.map((item, index) => <li key={`${item}-${index}`} className="flex gap-2.5 text-xs leading-5 text-[#66736c]"><span className={`mt-1 grid size-4 shrink-0 place-items-center rounded-full text-[9px] font-bold ${positive ? "bg-[#dceedd] text-[#3f7650]" : "bg-[#e9ede9] text-[#6d7872]"}`}>{numbered ? index + 1 : positive ? "✓" : "·"}</span><span>{item}</span></li>)}</ul> : <p className="text-xs text-[#9aa29e]">暂无</p>}</div>; }
function TagList({ items, muted }: { items: string[]; muted?: boolean }) { return <div className="flex flex-wrap gap-2">{items.map((item) => <span key={item} className={`rounded-full border px-2.5 py-1 text-[10px] font-semibold ${muted ? "border-[#e3e7e3] bg-[#f5f6f5] text-[#a3aaa6]" : "border-[#d9e5db] bg-[#edf5ee] text-[#416a53]"}`}>{item}</span>)}</div>; }
function ReadyCard({ ready, title, detail, action }: { ready: boolean; title: string; detail: string; action: () => void }) { return <button onClick={action} className="flex items-center gap-3 rounded-2xl border border-[#e0e5e0] p-4 text-left hover:bg-[#f8faf8]"><div className={`grid size-9 place-items-center rounded-xl ${ready ? "bg-[#dfeee0] text-[#447452]" : "bg-[#f0f1f0] text-[#9ba39e]"}`}>{ready ? <CheckCircle2 size={17} /> : <span className="text-xs font-bold">!</span>}</div><div className="min-w-0 flex-1"><p className="text-xs font-extrabold">{title}</p><p className="mt-1 truncate text-[11px] text-[#89928d]">{detail}</p></div><ChevronRight size={15} className="text-[#a7afaa]" /></button>; }
...
function ProfileLine({ icon: Icon, label }: { icon: React.ComponentType<{ size?: number; className?: string }>; label: string }) { return <div className="flex items-center gap-2.5 text-[11px] text-[#727e77]"><Icon size={14} className="shrink-0 text-[#83968b]" /><span className="truncate">{label}</span></div>; }
