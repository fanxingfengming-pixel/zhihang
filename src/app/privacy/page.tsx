  ...
  return (
    <AppShell>
      ...
      <article className="legal-card">
        <p className="legal-updated">更新日期：2026 年 9 月 10 日</p>
        <h2>我们处理的数据</h2>
        ...
        <h2>第三方模型</h2>
        <p>演示模式不会发送材料给模型服务商。真实模式只有在你明确同意并主动执行 Agent 时，才会把当前任务必要内容发送给你选择的 DeepSeek 或通义千问。自由对话中的 Career Profile 和当前 JD 默认关闭，只有逐项授权后才发送；页面会先展示字段范围，档案上下文不包含姓名和整份简历正文。服务商对请求数据的处理还受其自身条款和隐私政策约束。</p>
        <h2>保留、导出与删除</h2>
        <p>本机数据由浏览器存储控制；你可以在设置页下载备份或清除。云端工作区会保留到你删除云端数据或永久删除账号。删除账号后，数据库中的关联工作区、配额与模型用量记录会随账号删除。用量记录只保存请求编号、模型、Token、耗时、状态和可选成本，不保存求职资料正文或原始 IP。</p>
        <h2>安全边界</h2>
